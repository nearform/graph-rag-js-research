...
function optionalEnumField(enumValues, description, isRel) {
    if (enumValues && enumValues.length > 0) {
        return z.enum(enumValues).describe(`${description}. Available options are ${enumValues}`);
    } else {
        const nodeInfo =
            "Ensure you use basic or elementary types for node labels.\n" +
            "For example, when you identify an entity representing a person, " +
...
function createSimpleModel(nodeLabels = [], relTypes = []) {
    return z.object({
        nodes: z.array(z.object({
            id: z.string().describe('Name or human-readable unique identifier.'),
            type: optionalEnumField(nodeLabels, 'The type or label of the node.')
        })).describe('List of nodes'),
        relationships: z.array(z.object({
            sourceNodeId: z.string().describe('Name or human-readable unique identifier of source node'),
            sourceNodeType: optionalEnumField(nodeLabels, 'The type or label of the source node.'),
            targetNodeId: z.string().describe('Name or human-readable unique identifier of target node'),
            targetNodeType: optionalEnumField(nodeLabels, 'The type or label of the target node.'),
            type: optionalEnumField(relTypes, 'The type of the relationship.', true)
...
export class LLMGraphTransformer {
    constructor(llm, allowedNodes = [], allowedRelationships = [], prompt = defaultPrompt, strictMode = true) {
    ...
    async processResponse(document) {
        ...
        if (this.strictMode && (this.allowedNodes.length > 0 || this.allowedRelationships.length > 0)) {
            if (this.allowedNodes.length > 0) {
                ...
                relationships = relationships.filter(rel => this.allowedNodes.includes(rel.source.type) && this.allowedNodes.includes(rel.target.type));
            }
